...
function fetchApplications($conn, $uid) {
    $query = "SELECT b.name as bank_name, a.* FROM banks b LEFT JOIN applications a ON b.id = a.bank_id WHERE user_id=$uid";
...
// Fetch applications
$applications = fetchApplications($conn, $user_id);
...
<div class="bg-gray-100">
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4 text-center">Application Status</h1>
                ...
                <tbody class="bg-gray-100 text-center">
                    <?php foreach ($applications as $application): ?>
                        <tr class="hover:bg-gray-200">
                            <td class="border-b-2 border-r-2 px-3 py-3 font-bold">
                                <?php echo htmlspecialchars($application['bank_name']); ?>
                            </td>
                            <td class="border-b-2 px-3 py-3 font-bold flex justify-center items-center">
                                <?php
                                    switch ($application['status']) {
                                        case 'Approved':
                                            echo '<button class="block cursor-default rounded-md bg-green-400 hover:bg-green-500 px-4 py-2 font-medium backdrop-blur-lg text-center">Approved</button></td></tr>';
                                            break;
                                        case 'Rejected':
                                            echo '<button class="block cursor-default rounded-md bg-red-400 hover:bg-red-500 px-4 py-2 font-medium backdrop-blur-lg text-center">Rejected</button></td></tr>';
                                            break;
                                        case 'To Review':
                                            echo '<button class="block cursor-default rounded-md bg-orange-400 hover:bg-orange-500 px-4 py-2 font-medium backdrop-blur-lg text-center">In Review</button></td></tr>';
                                            break;
                                        default:
                                            echo 'Unknown';
                                    }
